...
with open("accuracy_data_compare_3_3.csv", newline="") as csv_file:
...
classical_avg = np.sum(classical_y_data) / 4
aer_avg = np.sum(aer_y_data) / 4
superstaq_avg = np.sum(superstaq_y_data) / 4

classical_std = np.std(classical_y_data) / np.sqrt(4)
aer_std = np.std(aer_y_data) / np.sqrt(4)
superstaq_std = np.std(superstaq_y_data) / np.sqrt(4)
...
plt.grid()
plt.figtext(.16,.75,"Train Size: 30\n Test Size: 15\n Shots: 1000\n Features: 4")
